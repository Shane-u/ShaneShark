...
- 主页与品牌展示：Hero 动效名片、技能/经历/项目/博客卡片，支持深浅色主题切换与响应式导航（桌面导航 + 移动端抽屉）。
- QA 知识库：标签筛选、栅格卡片、Lake 富文本详情页；提供新增/编辑/删除后台管理入口，口令弹窗校验后再进入。
- 收藏与书单：`Favorites` 收藏页与书单占位，方便扩展个性化内容。
- Sandbox Lab：在线多语言代码运行（Java/Python/C/C++/Go/Node.js），可填 STDIN，保存最近 50 条运行历史，编辑器亮/暗主题可独立切换。
- AI 复盘：调用 SiliconFlow，同“检验复盘”模型，对沙箱代码给出中文审查与优化建议。
- 部署与接口：前端静态化构建，可用 GitHub Pages/服务器部署；后端提供统一 `/api`，数据库默认 SQLite 文件存储。
...
- QA 列表 / 详情  
  ![QA 列表](./docs/imgs/QA.png)  
  ![QA 详情](./docs/imgs/details.png)
- 沙箱世界（在线多语言代码运行 + AI 复盘）  
  ![沙箱](./docs/imgs/sandbox.png)
- QA 检查流（校验与 AI 审查示意）  
  ![检查示意1](./docs/imgs/check1.png)  
  ![检查示意2](./docs/imgs/check_ai.png)
